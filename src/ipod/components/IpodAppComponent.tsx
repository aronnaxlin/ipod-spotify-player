...
import { useAppStoreShallow, useAudioSettingsStoreShallow, useIpodStoreShallow } from "@/stores/helpers";
import { useAudioSettingsStore } from "@/stores/useAudioSettingsStore";
import { useChatsStore } from "@/stores/useChatsStore";
import { Track, flushPendingLyricOffsetSave, getEffectiveTranslationLanguage, useIpodStore } from "@/stores/useIpodStore";
...
import { track } from "@/utils/analytics_mock";
...
import { BACKLIGHT_TIMEOUT_MS, SEEK_AMOUNT_SECONDS, formatKugouImageUrl, getYouTubeVideoId } from "../constants";
import { useLibraryUpdateChecker } from "../hooks/useLibraryUpdateChecker";
import type { RotationDirection, WheelArea } from "../types";
...
export function IpodAppComponent({
  ...
  // Spotify Auth Check - Handle PKCE callback
  useEffect(() => {
    const handleSpotifyCallback = async () => {
      // Check for authorization code in URL (PKCE flow - for web mode)
      ...
      if (code) {
        ...
        const success = await useSpotifyStore.getState().handleCallback(code);
        // Clear the URL parameters
        window.history.replaceState({}, document.title, window.location.pathname);
        if (success) {
          setMenuMode(true);
      ...
      // Legacy: Check for access_token in hash (Implicit flow - fallback)
      ...
      if (hash && hash.includes("access_token")) {
        ...
        if (token) {
          useSpotifyStore.getState().setAccessToken(token, expiresIn);
      ...
      // Initialize store
      useSpotifyStore.getState().init();
    };

    handleSpotifyCallback();

    // Listen for Electron IPC auth code from custom protocol
    if (window.electronAPI?.onSpotifyAuthCode) {
      window.electronAPI.onSpotifyAuthCode(async (code) => {
        console.log('Received Spotify auth code via IPC');
        const success = await useSpotifyStore.getState().handleCallback(code);
        if (success) {
          setMenuMode(true);
        }
    ...
    if (window.electronAPI?.onSpotifyAuthError) {
      window.electronAPI.onSpotifyAuthError((error) => {
        toast.error(`Spotify auth error: ${error}`);
      });
    }
  }, []);
  ...
  const spotifySeekThrottleRef = useRef<NodeJS.Timeout | null>(null);
  ...
  // Sync Spotify Now Playing - faster polling as backup for SDK events
  useEffect(() => {
    const spotifyStore = useSpotifyStore.getState();
    const syncInterval = 1000;

    const interval = setInterval(() => {
        if (spotifyStore.accessToken && spotifyStore.isActive) {
            spotifyStore.fetchPlaybackState().then(state => {
                // Force sync local state to match Spotify
                if (state && state.isPlaying !== isPlaying) {
  ...
  // Centralized playback action used across duplicate menus
  const playTrackAction = useCallback(async (track: Track, contextUri?: string) => {
      registerActivity();

      // Save current menu history before playing
      setMenuHistory((prev) => {
      ...
      const isSpotifyTrack = track.url?.startsWith('spotify:') || track.id?.includes(':');
      const spotifyStore = useSpotifyStore.getState();

      if (isSpotifyTrack && spotifyStore.accessToken) {
            // Play via Spotify
            // If we have a context URI (album/playlist), use it with offset
            // Otherwise just play the track URI
            await spotifyStore.playTrack(track.url, contextUri);

            // Update local state to reflect this track immediately (phantom track)
            // The syncNowPlaying will correct it later, but for responsiveness:
            const ipodStore = useIpodStore.getState();
            const existing = ipodStore.tracks.find(t => t.id === track.id);
            if (!existing) {
                ipodStore.setTracks([...ipodStore.tracks, track]);
            }
       ...
       // if not coming from "Now Playing", just go back in history
       setMenuHistory((prev) => {
              const updatedHistory = [...prev];
              return updatedHistory;
            });
  ...
  }, [registerActivity, setMenuHistory, setCurrentSongId, setIsPlaying, setMenuMode, showStatus, t]);
    ...
    return [
      {
        ...
        action: async () => {
          ...
          if (!spotifyStore.accessToken) {
              toast.error(t("apps.ipod.dialogs.loginRequired"));
              return;
          }

          toast.info("Loading Playlists...");
          await spotifyStore.fetchPlaylists();
          const playlists = spotifyStore.playlistsCache.items;
          ...
          const playlistMenu = playlists.map((playlist) => ({
            label: playlist.name,
            action: async () => {
                registerActivity();
                setMenuDirection("forward");
                ...
                const tracks = await spotifyStore.fetchPlaylistTracks(playlist.id);
                if (tracks.length === 0) {
                    toast.info("No tracks in playlist");
                    return;
                }

                const trackMenu = tracks.map((track) => ({
                    label: track.title,
                    action: () => playTrackAction(track, playlist.uri),
                    showChevron: false
                }));

                setMenuHistory((prev) => [
                    ...prev,
                    { title: playlist.name, items: trackMenu, selectedIndex: 0 }
                ]);
          ...
          // Add load more option for playlists
          const cache = spotifyStore.playlistsCache;
          if (cache.hasMore) {
            playlistMenu.push({
              label: `Load More (${cache.items.length}/${cache.total})...`,
              action: async () => {
                toast.info("Loading more playlists...");
                const morePlaylists = await spotifyStore.fetchPlaylists(true);

                // Rebuild menu
                const updatedPlaylistMenu = morePlaylists.map((playlist) => ({
                    label: playlist.name,
                    action: async () => {
                        registerActivity();
                        setMenuDirection("forward");
                ...
                const newCache = spotifyStore.playlistsCache;
                if (newCache.hasMore) {
                    updatedPlaylistMenu.push({
                      label: `Load More (${newCache.items.length}/${newCache.total})...`,
                      action: async () => {
                          const newerPlaylists = await spotifyStore.fetchPlaylists(true);
                          // Recursive rebuild would be needed here, simplified for now
                          // In a real app we'd abstract this menu builder
                          toast.info("Playlists loaded, please go back and re-enter menu to see all");
                      },
                      showChevron: false
                    });
                }
                ...
                setMenuHistory((prev) => {
                    ...
                    newHistory[newHistory.length - 1] = {
                        title: "Playlists",
                        items: updatedPlaylistMenu,
                        selectedIndex: Math.min(selectedMenuItem, updatedPlaylistMenu.length - 1),
                    };
                    return newHistory;
                });
              },
          ...
          setMenuHistory((prev) => [
            ...prev,
            { title: "Playlists", items: playlistMenu, selectedIndex: 0 },
          ]);
          setSelectedMenuItem(0);
        },
        showChevron: true,
      },
      {
        ...
        action: async () => {
          ...
          const albums = await spotifyStore.fetchSavedAlbums();
          if (albums.length === 0) {
              toast.info("No saved albums found");
              return;
          }

          const albumsMenu = albums.map((album) => ({
            label: album.name,
            action: async () => {
                registerActivity();
                setMenuDirection("forward");
                ...
                if (!tracks || tracks.length === 0) {
                    tracks = await spotifyStore.fetchAlbumTracks(album.id);
                }

                if (tracks.length === 0) {
                    await spotifyStore.playUri(album.uri);
                    setMenuMode(false);
                    return;
                }
                ...
                const trackMenu = tracks.map((track) => ({
                    label: track.title,
                    action: () => playTrackAction(track, album.uri),
                    showChevron: false
                }));

                setMenuHistory((prev) => [
                    ...prev,
                    { title: album.name, items: trackMenu, selectedIndex: 0 }
                ]);
                setSelectedMenuItem(0);
            },
            showChevron: true,
          ...
          setMenuHistory((prev) => [
            ...prev,
            { title: "Albums", items: albumsMenu, selectedIndex: 0 },
          ]);
          setSelectedMenuItem(0);
        },
        showChevron: true,
      },
      {
        label: "Artists",
         action: async () => {
          ...
          const artists = await spotifyStore.fetchFollowedArtists();
          if (artists.length === 0) {
              toast.info("No followed artists found");
              return;
          }

          const artistsMenu = artists.map((artist) => ({
            label: artist.name,
            action: async () => {
                registerActivity();
                setMenuDirection("forward");
                ...
                const artistAlbums = await spotifyStore.fetchArtistAlbums(artist.id);
                 if (artistAlbums.length === 0) {
                    toast.info("No albums found for artist");
                    return;
                }

                const albumListMenu = artistAlbums.map(album => ({
                    label: album.name,
                    action: async () => {
                        registerActivity();
                        setMenuDirection("forward");

                        // Fetch tracks for this artist album
                        const tracks = await spotifyStore.fetchAlbumTracks(album.id);
                ...
                setMenuHistory((prev) => [
                    ...prev,
                    { title: artist.name, items: albumListMenu, selectedIndex: 0 }
                ]);
                setSelectedMenuItem(0);
            },
            showChevron: true,
          ...
          // Add load more for Artists
          const cache = spotifyStore.artistsCache;
          if (cache.hasMore) {
            artistsMenu.push({
              label: `Load More (${cache.items.length}/${cache.total})...`,
              action: async () => {
                toast.info("Loading more artists...");
                const moreArtists = await spotifyStore.fetchFollowedArtists(true);

                 // Rebuild menu
                 const updatedArtistsMenu = moreArtists.map((artist) => ({
                    label: artist.name,
                    action: async () => {
                        registerActivity();
                        setMenuDirection("forward");
                 ...
                 const newCache = spotifyStore.artistsCache;
                 if (newCache.hasMore) {
                     updatedArtistsMenu.push({
                         label: `Load More (${newCache.items.length}/${newCache.total})...`,
                         action: async () => {
                             await spotifyStore.fetchFollowedArtists(true);
                             toast.info("Artists loaded, please go back and re-enter menu");
                         },
                         showChevron: false
                     });
                 }
                 ...
                 setMenuHistory((prev) => {
                    ...
                    newHistory[newHistory.length - 1] = {
                        title: "Artists",
                        items: updatedArtistsMenu,
                        selectedIndex: Math.min(selectedMenuItem, updatedArtistsMenu.length - 1),
                    };
                    return newHistory;
                 });
              },
          ...
          setMenuHistory((prev) => [
            ...prev,
            { title: "Artists", items: artistsMenu, selectedIndex: 0 },
          ]);
          setSelectedMenuItem(0);
        },
        showChevron: true,
      },
      {
        ...
        action: async () => {
          ...
          const songs = await spotifyStore.fetchLikedSongs();
          if (songs.length === 0) {
              toast.info("No liked songs found");
              return;
          }

          const songsMenu = songs.map((song) => ({
            label: song.title,
            action: () => playTrackAction(song),
            showChevron: false,
          }));

          // Add "Load More" option if there are more songs
          const cache = spotifyStore.likedSongsCache;
          if (cache.hasMore) {
            songsMenu.push({
              label: `Load More (${cache.items.length}/${cache.total})...`,
              action: async () => {
                toast.info("Loading more songs...");
                const moreSongs = await spotifyStore.fetchLikedSongs(true);
                // Rebuild menu with new songs
                const updatedSongsMenu = moreSongs.map((song) => ({
                  label: song.title,
                  action: () => playTrackAction(song),
                  showChevron: false,
                }));
                const newCache = spotifyStore.likedSongsCache;
                if (newCache.hasMore) {
                  updatedSongsMenu.push({
                    label: `Load More (${newCache.items.length}/${newCache.total})...`,
                    action: async () => {
                      // Recursive load more
                      await spotifyStore.fetchLikedSongs(true);
                    },
                    showChevron: false,
                  });
                }
                setMenuHistory((prev) => {
                  ...
                  newHistory[newHistory.length - 1] = {
                    title: "Songs",
                    items: updatedSongsMenu,
                    selectedIndex: Math.min(selectedMenuItem, updatedSongsMenu.length - 1),
                  };
                  return newHistory;
                });
              },
          ...
          setMenuHistory((prev) => [
            ...prev,
            { title: "Songs", items: songsMenu, selectedIndex: 0 },
          ]);
          setSelectedMenuItem(0);
        },
        showChevron: true,
  ...
  }, [tracks, registerActivity, setCurrentSongId, setIsPlaying, isOffline, showOfflineStatus, t, showStatus]);
    ...
    return [
      {
        ...
        action: async () => {
            ...
            if (spotifyState.accessToken) {
                spotifyState.disconnect();
                showStatus("Disconnected");
            } else {
                await spotifyState.login();
                toast.info('Login in browser, then use "Paste Auth URL" below', { duration: 10000 });
            }
        },
        showChevron: false,
        value: isSpotifyActive ? "Connected" : "Connect",
      },
      {
        label: "Paste Auth URL",
        action: async () => {
            try {
              // Read from clipboard
              const url = await navigator.clipboard.readText();

              // Check if it's the redirect URL (contains code= and starts with our redirect URI)
              if (url && url.includes('127.0.0.1:5173') && url.includes('?code=')) {
                const success = await useSpotifyStore.getState().handlePastedUrl(url);
                if (success) {
                  showStatus("Connected!");
                }
              } else if (url && url.includes('accounts.spotify.com')) {
                toast.error('Wrong URL! Copy the URL AFTER login (starts with 127.0.0.1)', { duration: 5000 });
              } else {
                toast.error('Copy the redirect URL from browser (starts with http://127.0.0.1:5173/?code=...)', { duration: 5000 });
              }
            } catch (e) {
              toast.error('Failed to read clipboard. Please copy the URL first.');
            }
        },
        showChevron: false,
        value: "",
  ...
  }, [loopCurrent, loopAll, isShuffled, backlightOn, theme, memoizedToggleRepeat, memoizedToggleShuffle, memoizedToggleBacklight, memoizedHandleThemeChange, t, isSpotifyActive, showStatus]);
  ...
  const mainMenuItems = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        action: () => {
          registerActivity();
          // Local playback for YouTube items not implemented in this cleanup
          // Just do nothing or log error
          console.warn("YouTube video playback is deprecated");
      ...
      },
      {
        ...
        action: async () => {
          ...
          const spotifyStore = useSpotifyStore.getState();
          if (spotifyStore.accessToken) {
            await spotifyStore.playLikedSongs(true);
            setMenuMode(false);
            setIsPlaying(true);
          } else {
  ...
  }, [registerActivity, musicMenuItems, settingsMenuItems, memoizedToggleShuffle, memoizedToggleBacklight, t]);
  ...
  const rebuildMenuItems = useCallback((menu: typeof menuHistory[0]): typeof menuHistory[0]["items"] | null => {
    ...
    return null;
  }, [mainMenuItems, musicMenuItems, settingsMenuItems, tracks, t, registerActivity, isOffline, showOfflineStatus, setCurrentSongId, setIsPlaying]);
  ...
  const handleAddTrack = useCallback(
    async (url: string) => {
      ...
      try {
        ...
        // if (addedTrack) {
        if (true) {
           // TODO: Restore addTrackFromVideoId or implement replacement
           toast.info("Add YouTube track functionality temporarily disabled");
           return;
  ...
  const handleMenuButton = useCallback(() => {
    ...
    if (isPlaying && tracks[currentIndex]) {
        showStatus("Playing");
    }
    if (menuMode) {
  ...
  }, [playClickSound, vibrate, registerActivity, isCoverFlowOpen, menuMode, menuHistory, mainMenuItems, musicMenuItems, tracks, currentIndex, cameFromNowPlayingMenuItem, isOffline, showOfflineStatus, setCurrentSongId, setIsPlaying, t]);

  const handleMenuSelect = useCallback(() => {
    if (!menuMode) return;
    const currentMenu = menuHistory[menuHistory.length - 1];
    if (currentMenu && currentMenu.items[selectedMenuItem]) {
      const item = currentMenu.items[selectedMenuItem];
      if (item.action) {
        playClickSound();
        vibrate();
        registerActivity();
  ...
  }, [menuMode, menuHistory, selectedMenuItem, playClickSound, vibrate, registerActivity]);
  ...
  const handleCenterLongPress = useCallback(() => {
    ...
    // Only toggle cover flow if we have albums to show
    if (tracks.length > 0) {
      setIsCoverFlowOpen((prev) => !prev);
    }
  }, [playClickSound, vibrate, tracks.length]);
  ...
  const handleTogglePlay = useCallback(() => {
    if (isSpotifyActive) {
      const spotifyStore = useSpotifyStore.getState();
      // Use Store state as truth, fall back to false if null
      const isSpotifyPlaying = spotifyStore.playbackState?.isPlaying ?? false;
      ...
      setIsPlaying(!isSpotifyPlaying);
      showStatus(!isSpotifyPlaying ? "▶" : "⏸︎");
    } else {
      togglePlay();
    }
  }, [isSpotifyActive, togglePlay, setIsPlaying, showStatus]);
  ...
  const handleWheelClick = useCallback(
    (area: WheelArea) => {
      ...
      switch (area) {
        ...
        case "left": // Prev
          if (menuMode) {
             ...
             setMenuHistory((prev) => {
               ...
               const parentMenu = newHist[newHist.length - 1];
               if (parentMenu) {
                 setSelectedMenuItem(parentMenu.selectedIndex || 0);
               }
               return newHist;
             });
          } else {
            // Now playing: prev track
            if (isSpotifyActive) {
                useSpotifyStore.getState().previous();
                showStatus("⏮");
            } else {
        ...
        case "right": // Next
          ...
          break;
        case "bottom": // Play/Pause
          if (menuMode) {
            // Do nothing in menu mode for bottom button usually, or jump to now playing
            if (currentSongId) {
               setMenuMode(false);
            }
          } else {
            // Toggle play/pause - centralized logic
            handleTogglePlay();
            // Status will be updated by effect, but show optimistic toast
        ...
        case "center":
          ...
          if (menuMode) {
            ...
            // In Now Playing mode: toggle play
    ...
    [
      menuMode,
      handleMenuButton, // Added for 'top' case
      previousTrack,
      nextTrack,
      handleTogglePlay,
      showStatus,
      ...
      tracks,
      currentIndex,
      isCoverFlowOpen, // For cover flow selection
      menuHistory, // For menu selection
      selectedMenuItem, // For menu selection
      startTrackSwitch // For prev/next track
    ]
  );
  ...
  const handleWheelRotation = useCallback(
    (direction: RotationDirection) => {
      ...
      if (menuMode) {
        ...
        // Now playing: seek
        // Strict separation of Spotify vs Local logic
        if (isSpotifyActive) {
          const spotifyStore = useSpotifyStore.getState();
          // Fallback to 0 if playbackState is missing but we're active
          const currentPosition = spotifyStore.playbackState?.positionMs ?? 0;
          const duration = spotifyStore.playbackState?.durationMs ?? 180000; // default 3m if unknown
          const seekAmount = 5000; // 5 seconds

          let newPosition: number;
          if (direction === "clockwise") {
            newPosition = Math.min(currentPosition + seekAmount, duration);
          } else {
            newPosition = Math.max(0, currentPosition - seekAmount);
          }

          // 1. Optimistic UI update via Store (so invalidates old state)
          ...
          // 2. Throttle the actual API call
          if (spotifySeekThrottleRef.current) {
            clearTimeout(spotifySeekThrottleRef.current);
          }

          spotifySeekThrottleRef.current = setTimeout(() => {
             spotifyStore.seekToPosition(newPosition);
          }, 300); // 300ms throttle

          const newTimeSeconds = newPosition / 1000;
          showStatus(
            `${direction === "clockwise" ? "⏩︎" : "⏪︎"} ${Math.floor(newTimeSeconds / 60)}:${String(Math.floor(newTimeSeconds % 60)).padStart(2, "0")}`
          );
        } else {
          // Local playback seek
          const activePlayer = isFullScreen ? fullScreenPlayerRef.current : playerRef.current;
    ...
    [playScrollSound, registerActivity, menuMode, menuHistory, isFullScreen, showStatus, isCoverFlowOpen, isSpotifyActive]
  );
  ...
  // Prioritize Spotify playbackState when active for accurate track display
  const currentTrack = useMemo(() => {
    // When Spotify is active and has playbackState, create track from it
    if (isSpotifyActive && spotifyPlaybackState && spotifyPlaybackState.trackId) {
      return {
        id: spotifyPlaybackState.trackId,
        title: spotifyPlaybackState.trackName || 'Unknown',
        artist: spotifyPlaybackState.artistName || 'Unknown Artist',
        album: spotifyPlaybackState.albumName || 'Unknown Album',
        url: `spotify:track:${spotifyPlaybackState.trackId}`,
        cover: spotifyPlaybackState.albumCover || '',
        duration: spotifyPlaybackState.durationMs,
      } as Track;
    }
    // Fallback to tracks array
    return tracks[currentIndex] ?? null;
  }, [isSpotifyActive, spotifyPlaybackState, tracks, currentIndex]);
  ...
  return (
    <div className="flex flex-col w-full h-full overflow-hidden">
      ...
      >
        <div
          ref={containerRef}
          className="ipod-force-font flex flex-col items-center justify-center w-full h-full p-4 select-none"
          style={{
            position: "relative",
            overflow: "hidden",
        ...
        >
          <div
            className={cn(
              ...
              theme === "classic" ? "bg-[#f2f2f2]" : "bg-[#1c1c1c]"
            )}
            style={{
          ...
          >
            <div
              ...
              style={{ height: "150px", minHeight: "150px", maxHeight: "150px", WebkitAppRegion: "no-drag" } as any}
            >
              <IpodScreen
